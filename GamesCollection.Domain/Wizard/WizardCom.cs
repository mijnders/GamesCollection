...
namespace GamesCollection.Domain.Wizard
{
    public class WizardCom
    {
        public static int CountPrediction(WizardPlayer player, string trump)
        {
            ...
            foreach (var playerOnHandCard in player.OnHandCards)
            {
                copyMainDeck.Deck.Remove(playerOnHandCard);
            }
        ...
        public static WizardCard PlayCard(WizardPlayer player, string trump, WizardCardDeck stack, List<int> playedBy)
        {
            ...
            var listList = new List<List<WizardCard>>
            {
                ...
                player.OnHandCards.Where(card => card.Value is 14 or 0).ToList(),
                player.OnHandCards
            };
            foreach (var list in listList.Where(list => list.Any()))
            {
                ...
                if (list.Count == 1 && list.First().Species == speciesToServe && list.First().Value != 14 &&
                    list.First().Value != 0 && !listList[2].Any()) return list.First();
                foreach (var playerOnHandCard in list)
                {
                    ...
                    if (playOffensive)
                    {
                        ...
                        if (listList[2].Any(card => card.Value == 14))
                            return player.OnHandCards.First(card => card.Value == 14);
                    }
                    else
                    {
                        if (WizardLogic.CheckPlayedCards(playedBy, stack, trump) != player.Id)
                        ...
                        if (listList[2].Any(card => card.Value == 0))
                            return player.OnHandCards.First(card => card.Value == 0);
                    }
        ...
        private static WizardCard GetLowestCard(IReadOnlyCollection<WizardCard> cards)
        {
            ...
            foreach (var card in cards.Where(card => card.Value < lowestCard.Value))
            {
                lowestCard = card;
            }
